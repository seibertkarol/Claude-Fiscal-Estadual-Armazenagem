...
- **AMARELO** — o retorno e suas remessas até fechariam matematicamente, mas o
  **ICMS declarado no XML não bate com o valor lançado no SAP** (erro de
  ...
  manual.
- **VERDE** — **NF duplicada**: o mesmo número de NF aparece em doc_saps
  diferentes no ZSD. O algoritmo de conciliação não consegue processar esses
  casos corretamente (agrupa por NF, não por doc_sap), então marca em verde
  para revisão manual. A coluna I dessas linhas mostra o **doc_sap** (em vez
  do número da NF) para que a usuária consiga identificar qual remessa pertence
  a qual retorno.
...
- **Coluna I — NOTA DE RETORNO**: número(s) da(s) nota(s) de retorno que casaram
  ...
  Para linhas **verdes** (NF duplicada), mostra o **doc_sap** em vez do número da
  NF — nas linhas de retorno, o doc_sap da própria linha; nas remessas, o doc_sap
  do retorno que a referencia (determinado via chave de acesso do XML cruzada com
  o ZSD)
...
RESUMO DOS RETORNOS:
  ...
  Divergente ICMS:   X     ← XML != valor lançado no SAP
  Sem retorno P6:    X     ← XMLs sem lançamento na planilha

RESUMO DAS REMESSAS PINTADAS:
  Rosa:    X   Laranja: X   Amarelo: X   Verde: X

DIAGNOSTICO MONETARIO:
  ROSA saldo liquido: R$X  ← deve ser ~R$0
  VERDE saldo liquido: R$X ← NF duplicada, revisão manual
```
